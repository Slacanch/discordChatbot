...
class Sfx:
    ...
    async def ask(self, ctx, *text: str):
        """ask sborrabot what he thinks about a topic"""
        ...
        question =  " ".join(text)
        sborraAnswer = subprocess.Popen(f'python3.6 ~/Projects/discordBot/red-discordbot/cogs/sborraAnswer.py "{question}"',
                                        stdout=subprocess.PIPE, shell=True)
        while True:
            await asyncio.sleep(40)
            ans = sborraAnswer.stdout.read()
            if ans is not None:
                resp = "qualcuno chiede: "+question+". "+ans.decode()
                self.enqueue_tts(vchan, resp)
                break
#                if self.enqueue_tts(vchan, resp):
                    #await self.bot.say("sborra-bot risponde: "+ans.decode())
#                    return
#                else:
#                    return

#        resp = self.sborraBot.get_response(" ".join(text))
#        self.enqueue_tts(vchan, "qualcuno chiede: "+" ".join(text)+"."+str(resp))
#        await self.bot.say("la saggezza di Sborra Bot: " + str(resp))
